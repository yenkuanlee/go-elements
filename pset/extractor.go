...
// The Extractor requires provision of a single PSET
...
// instance. Note that if the PSET is in-complete, then an error
// ErrIncompletePSET will be returned. As the extracted transaction has been
...
func Extract(p *Pset) (*transaction.Transaction, error) {
	...
	if !p.IsComplete() {
		return nil, psbt.ErrIncompletePSBT
	}
	...
	// sigScript data.
	for i, tin := range finalTx.Inputs {
		...
		if pInput.FinalScriptSig != nil {
			tin.Script = pInput.FinalScriptSig
		}
		...
		if pInput.FinalScriptWitness != nil {
			...
			tin.Witness = make(transaction.TxWitness, witCount)
